...
class Music(commands.Cog):
    ...
    async def join(self, ctx):
        ...
        try:
            message = await self._join_voice_channel(ctx)
    ...
    async def _play_next_song(self, ctx):
        ...
        song_info = self.bot.server_data[ctx.guild.id].next_song()  # Dequeue the next song & return its data

        if not song_info:
            # Queue is empty, so clear now playing.
            if ctx.guild.id in self.bot.server_data:
                self.bot.server_data[ctx.guild.id].nowplaying = {}
                return
        
        title, url = song_info
        if 'spotify' in url:
            url = self._get_youtube_url(title)

        player = await YTDLSource.from_url(url, loop=self.bot.loop)

        async def after_callback(ctx, e, player):
            await self._song_finished(ctx, e, player)
    ...
    async def _song_finished(self, ctx, error, player):
        """Called when a song finishes playing"""
        song_title = player.title
        player.cleanup()
        self._file_cleanup(song_title)
        if error:
            print(f"Player error: {error}")

        # Logic for bot.server_data who have 'loop' enabled.
        if self.bot.server_data[ctx.guild.id].settings['loop']:
            async def loop_song():
                self.bot.server_data[ctx.guild.id].enqueue(player.title, player.data['original_url'])
                await self._play_next_song(ctx)
            await loop_song()
        else:
            await self._play_next_song(ctx)

    def _file_cleanup(self, filename_base):
    ...
    async def _extract_playlist_info(self, playlist_url):
        """Returns a list of videos from a given Youtube playlist URL."""
        # Get playlist info without downloading the videos
        playlist_info = await self.bot.loop.run_in_executor(None, lambda: ytdl_search.extract_info(playlist_url, download=False))
    ...
    def _parse_spotify_link(self, url):
        """Returns a list of tracks from a given Spotify playlist, album, or track."""
        token = self._get_spotify_token(self.bot.config["spotify_id"], self.bot.config["spotify_secret"])
        headers = {
    ...
    def _get_youtube_url(self, query):
    ...
    async def _process_query(self, ctx, query):
        ...
        if not self._is_url(query):
            ...
            url = self._get_youtube_url(query)
            if url:
                query = url
            else:
        ...
        if 'spotify' in query:
            try:
                tracks = self._parse_spotify_link(query)
                for track in tracks:
                    self.bot.server_data[ctx.guild.id].enqueue(track, query)
                await ctx.send(f"{len(tracks)} tracks have been added to the queue!")
            except Exception as e:
                await ctx.send(e)
            return
        elif 'playlist?' in query:
            videos = await self._extract_playlist_info(query)
            for video in videos:
                self.bot.server_data[ctx.guild.id].enqueue(video['title'], video['url'])
        ...
        self.bot.server_data[ctx.guild.id].enqueue(player.title, query)
